...
export const metadata = { title: 'Attendance — Royal Gold Banquet' };
...
export default async function AttendancePage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return (
    <AttendanceClient
      ...
      days={matrix.days}
      employees={matrix.employees.map((e: any) => ({ id: e.id, name: e.name, designation: e.designation, salary: Number(e.monthly_salary) }))}
      cells={matrix.cells}
