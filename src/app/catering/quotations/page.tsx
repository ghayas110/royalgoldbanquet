...
import { Plus, FileText, ArrowUpRight, Search } from 'lucide-react';

export const metadata = { title: 'Quotations — Catering' };
export const dynamic = 'force-dynamic';

const STATUSES = ['QUOTATION', 'CONFIRMED', 'COMPLETED', 'CANCELLED'];

export default async function CateringQuotationsPage({
  searchParams,
  ...
  const rows = await getCateringQuotations({ status, docType: 'QUOTATION', search: sp.q?.trim() || undefined });
  ...
  const href = (s: string) => `/catering/quotations${s === 'ALL' ? '' : `?status=${s}`}`;

  return (
    <div className="space-y-6">
      <SectionTitle
        eyebrow="Catering"
        sub="Every quotation issued, newest first."
        right={canManage ? <Link href="/catering/quotations/new"><Button><Plus className="mr-1.5 h-4 w-4" /> New quotation</Button></Link> : undefined}
      >
        Quotations
      </SectionTitle>
